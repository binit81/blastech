...
<table  id="view_stock_recorddata" class="table tablesaw table-bordered table-hover table-striped mb-0" data-tablesaw-mode="swipe" data-tablesaw-sortable data-tablesaw-minimap data-tablesaw-mode-switch>
    ...
    @foreach($inward_product AS $key => $value)
        ...
        $value->pending_qty_total_cost = 0;
        if(isset($value->pending_qty_total_cost) && $value->pending_qty_total_cost != '')
        {
            $value->pending_qty_total_cost = $value->pending_qty_total_cost;
        }
    ...
    $(document).ready(function(e){
        $(document).ready(function(e){
           ...
           // $Total_cost_rate = (($total_pending_qty_total_cost + $opening_qty_total_cost) )/($total_instock);
            ...
            $('.opening').html(0);
            $('.totalinwardqty').html(0);
            $('.totalsoldqty').html(0);
            $('.totalrestockqty').html(0);
            $('.totaldamageqty').html(0);
            $('.totalusedqty').html(0);
            $('.totalsupprqty').html(0);
            $('.totalinstock').html(0);
            $('.totalinstock_cost').html(0);
